...
export async function POST(request) {
  try {
    ...
    return json({ file: { name: originalName, url: `/uploads/library-items/${storedName}`, size: Number(file.size || 0), type: String(file.type || "application/octet-stream") } }, 201);
  } catch (error) {
    console.error("library_item_upload_failed", error);
