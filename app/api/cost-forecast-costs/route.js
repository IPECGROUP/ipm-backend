...
async function ensureCostForecastTables() {
  await prisma.$executeRawUnsafe(`
    CREATE TABLE IF NOT EXISTS "cost_forecast_projects" (
      "id" SERIAL NOT NULL,
  ...
  await prisma.$executeRawUnsafe(`
    CREATE TABLE IF NOT EXISTS "cost_forecast_values" (
      "id" SERIAL NOT NULL,
      "project_id" INTEGER NOT NULL,
  ...
  await prisma.$executeRawUnsafe(`
    CREATE TABLE IF NOT EXISTS "cost_forecast_items" (
      "id" SERIAL NOT NULL,
      "project_id" INTEGER NOT NULL,
  ...
  await prisma.$executeRawUnsafe(`
    CREATE UNIQUE INDEX IF NOT EXISTS "cost_forecast_projects_project_id_key"
    ON "cost_forecast_projects"("project_id")
  `);
  await prisma.$executeRawUnsafe(`
    CREATE INDEX IF NOT EXISTS "cost_forecast_projects_project_id_idx"
    ON "cost_forecast_projects"("project_id")
  `);
  await prisma.$executeRawUnsafe(`
    CREATE UNIQUE INDEX IF NOT EXISTS "cost_forecast_values_project_id_budget_code_month_key_key"
    ON "cost_forecast_values"("project_id", "budget_code", "month_key")
  `);
  await prisma.$executeRawUnsafe(`
    CREATE INDEX IF NOT EXISTS "cost_forecast_values_project_id_idx"
    ON "cost_forecast_values"("project_id")
  `);
  await prisma.$executeRawUnsafe(`
    CREATE INDEX IF NOT EXISTS "cost_forecast_values_project_id_budget_code_idx"
    ON "cost_forecast_values"("project_id", "budget_code")
  `);
  await prisma.$executeRawUnsafe(`
    CREATE UNIQUE INDEX IF NOT EXISTS "cost_forecast_items_project_id_code_key"
    ON "cost_forecast_items"("project_id", "code")
  `);
  await prisma.$executeRawUnsafe(`
    CREATE INDEX IF NOT EXISTS "cost_forecast_items_project_id_idx"
    ON "cost_forecast_items"("project_id")
  `);

  await prisma.$executeRawUnsafe(`
    DO $$ BEGIN
      IF NOT EXISTS (
        SELECT 1 FROM pg_constraint WHERE conname = 'cost_forecast_projects_project_id_fkey'
      ) THEN
  ...
  await prisma.$executeRawUnsafe(`
    DO $$ BEGIN
      IF NOT EXISTS (
        SELECT 1 FROM pg_constraint WHERE conname = 'cost_forecast_values_project_id_fkey'
      ) THEN
  ...
  await prisma.$executeRawUnsafe(`
    DO $$ BEGIN
      IF NOT EXISTS (
        SELECT 1 FROM pg_constraint WHERE conname = 'cost_forecast_items_project_id_fkey'
      ) THEN
...
export async function GET(request) {
  const denied = await requirePagePermission(request, "پیش‌بینی جریان نقدی", "پیش‌بینی هزینه‌ها");
  if (denied) return denied;
  try {
    await ensureCostForecastTables();

    const projects = await prisma.$queryRaw`
      SELECT project_id, created_at, updated_at
      FROM cost_forecast_projects
      ORDER BY id ASC
    `;

    const values = await prisma.$queryRaw`
      SELECT project_id, budget_code, month_key, amount, updated_at
      FROM cost_forecast_values
    ...
    console.error("cost_forecast_costs_get_error", e);
    return json({ error: "internal_error" }, 500);
  }
}

export async function POST(req) {
  const denied = await requirePagePermission(req, "پیش‌بینی جریان نقدی", "پیش‌بینی هزینه‌ها");
  if (denied) return denied;
  try {
    await ensureCostForecastTables();
    ...
    if (body?.action === "add_item") {
      ...
      const rows = await prisma.$queryRaw`
        INSERT INTO cost_forecast_items (project_id, parent_code, code, title, row_index, updated_at)
    ...
    await prisma.$executeRaw`
      INSERT INTO cost_forecast_projects (project_id, updated_at)
    ...
    console.error("cost_forecast_costs_post_error", e);
    return json({ error: "internal_error" }, 500);
  }
}
...
export async function PATCH(req) {
  ...
  try {
    ...
    await prisma.$executeRaw`
      INSERT INTO cost_forecast_values (project_id, budget_code, month_key, amount, updated_at)
    ...
    console.error("cost_forecast_costs_patch_error", e);
    return json({ error: "internal_error" }, 500);
  }
}
...
export async function DELETE(req) {
  ...
  try {
    ...
    if (budgetCode) {
      await prisma.$executeRaw`
        DELETE FROM cost_forecast_values
        WHERE project_id = ${projectId} AND budget_code = ${budgetCode}
      `;
      if (itemId) {
        await prisma.$executeRaw`
          DELETE FROM cost_forecast_items
      ...
      `;
      await prisma.$executeRaw`
        DELETE FROM cost_forecast_projects
        WHERE project_id = ${projectId}
      `;
    }
    ...
    console.error("cost_forecast_costs_delete_error", e);
    return json({ error: "internal_error" }, 500);
  }
}
