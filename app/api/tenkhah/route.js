...
async function requests(where = "", params = []) {
  return prisma.$queryRawUnsafe(`SELECT t.id,t.request_number AS "requestNumber",t.request_date AS "requestDate",t.project_id AS "projectId",t.requested_amount::text AS "requestedAmount",t.currency,
    ...
    t.payment_request_id AS "paymentRequestId",t.created_by_id AS "createdById",COALESCE(t.beneficiary_user_id,t.created_by_id) AS "beneficiaryUserId",beneficiary.name AS "beneficiaryName",beneficiary.username AS "beneficiaryUsername",t.project_manager_id AS "projectManagerId",t.finance_user_id AS "financeUserId",t.current_assignee_user_id AS "currentAssigneeUserId",t.stage,t.status,t.manager_approved_date AS "managerApprovedDate",t.project_liquidity::text AS "projectLiquidity",t.charged_date AS "chargedDate",t.charged_amount::text AS "chargedAmount",p.code AS "projectCode",p.name AS "projectName",creator.name AS "requesterName",creator.username AS "requesterUsername",assignee.name AS "currentAssigneeName" FROM tenkhah_requests t LEFT JOIN projects p ON p.id=t.project_id LEFT JOIN "User" creator ON creator.id=t.created_by_id LEFT JOIN "User" beneficiary ON beneficiary.id=COALESCE(t.beneficiary_user_id,t.created_by_id) LEFT JOIN "User" assignee ON assignee.id=t.current_assignee_user_id ${where} ORDER BY t.created_at DESC`, ...params);
}
...
export async function POST(r) {
  try {
    ...
    await prisma.$executeRawUnsafe("INSERT INTO tenkhah_requests (payment_request_id,request_number,request_date,project_id,requested_amount,currency,unregistered_balance,unsettled_balance,created_by_id,beneficiary_user_id,project_manager_id,finance_user_id,current_assignee_user_id,project_liquidity,stage) VALUES ($1,$2,$3,$4,$5::bigint,$6,$7::bigint,$8::bigint,$9,$10,$11,$12,$13,$14::bigint,$15)", linkedPayment.id, requestNumber, String(b.requestDate), pid, String(w), String(b.currency || ""), String(w), String(w), uid, beneficiaryId, initialAssigneeId, null, initialAssigneeId, String(amount(b.projectLiquidity)), initialStage);
