...
export async function POST(request) {
  try {
    ...
    const project = await prisma.project.findFirst({ where: { id: projectId, isActive: true } }); if (!project || !/^\d{3}$/.test(String(project.code || "").trim())) return json({ error: "active_project_required" }, 400);
...
export async function PATCH(request) {
  try {
    ...
    const project = await prisma.project.findFirst({ where: { id: projectId, isActive: true } }); if (!project || !/^\d{3}$/.test(String(project.code || "").trim())) return json({ error: "active_project_required" }, 400); await ensureSchema(); const files = filesOf(body.files);
