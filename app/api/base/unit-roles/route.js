...
export async function GET() {
  try {
    ...
    return Response.json({
      ok: true,
      items: units.map(mapUnit),
      units: units.map((u) => ({ id: u.id, name: u.name, label: u.name, code: u.code })),
      roles,
    });
  } catch (e) {
...
export async function POST(request) {
  try {
    ...
    if (!hasRole) {
      ...
      return Response.json({ ok: true, item: mapUnit(item), unit, role: null });
    }
    ...
    await prisma.unitRoleMap.upsert({
      where: {
        unitId_roleId: {
          unitId: unit.id,
          roleId: role.id,
    ...
    return Response.json({ ok: true, item: mapUnit(item), unit, role });
  } catch (e) {
...
export async function DELETE(request) {
  try {
    ...
    if (roleId && Number.isFinite(roleId)) {
      await prisma.unitRoleMap.delete({
        where: {
          unitId_roleId: { unitId, roleId },
        },
      });
    } else {
      await prisma.unitRoleMap.deleteMany({ where: { unitId } });
    }
