...
import { isSessionExpired } from "./security";
...
export async function hasPagePermission(request, page, permission) {
  ...
  if (!user || user.isActive === false || (user.expiresAt && new Date(user.expiresAt).getTime() < Date.now()) || isSessionExpired(session)) return false;
