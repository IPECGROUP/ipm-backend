...
export async function hasPagePermission(request, page, permission) {
  ...
  if (!user || user.isActive === false || (user.expiresAt && new Date(user.expiresAt).getTime() < Date.now()) || (session.expiresAt && new Date(session.expiresAt).getTime() < Date.now())) return false;
